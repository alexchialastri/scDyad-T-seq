...
my $fastafileR2In = $ARGV[3];
open($fastafileR2, "$fastafileR2In");


my $outputR1In = substr($fastafileR1In,0,-6)."-BS_of_CpGDiad.fastq";
my $outputR2In = substr($fastafileR2In,0,-6)."-BS_of_CpGDiad.fastq";



open($outputR1, '>', "$outputR1In");
open($outputR2, '>', "$outputR2In");
...
$SequencedCellBarcode = substr($sequence,$PCRCodeLength+$UMILength,$BarcodeLength); #20 because PCR barcode is 16 long and the UMI is 4 [DWDD]
	...
	if ($start1a >= $BarcodeLength-1)
	{
	    ...
	    $headerR22 = $headerR2 . '-:' . $UMIBC.$BarCode[$i];
		...
		print $outputR2 ("$headerR22\n$sequenceR2$plusR2$qualR2");
		last;
		}
